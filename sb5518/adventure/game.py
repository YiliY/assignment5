...
#Adding two objects with different data types for question 2

possible_yes_answers = ('y','yes')
possible_no_answers = ('n','no')
...
class Game(Data):
    ...
    def _do_command(self, words):
        if self.yesno_callback is not None:
            ...
            #Old yes/no check statement
            
                  #answer = YESNO_ANSWERS.get(words[0], None)
            
            #New yes/no check statements
            
            if words[0] in possible_yes_answers:
                answer = True
            elif words[0] in possible_no_answers:
                answer = False
            else:
                answer = None
