...
#define valid input for yes and no
VALIDYES = ['y', 'yes']
VALIDNO = ['n', 'no']
VALIDYESANDNO = ['y', 'yes', 'n', 'no']
...
class Game(Data):
    ...
    def _do_command(self, words):
	    ...
	    #get rid of the YESNO_ANSWERS dictionary
	    #if the first word is not valid y,yes,n,no
            if words[0] not in VALIDYESANDNO:
		#for invalid input, ask for another input using the 
		#original code
                ...
                self.yesno_callback = None
		#if valid input for y or yes, callback True
		if words[0] in VALIDYES:
			callback(True)
		#if valid input for n or no, callback False
		elif words[0] in VALIDNO:
			callback(False)
                return
        if self.is_dead:
