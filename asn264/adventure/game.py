...
YESNO_OPTIONS = [ ['y', True], ['yes', True], ['n', False], ['no', False] ]
...
class Game(Data):
    ...
    def _do_command(self, words):
        if self.yesno_callback is not None:
            ...
            #set default value of answer to None
            answer = None 
            for yesno_variation in range(0,len(YESNO_OPTIONS)):
                 #YESNO_OPTIONS is a list of lists where the 0th entry of each list is a valid answer to the y/n question
                 #The 1st entry in each list is a Boolean just like in the original dictionary
                 if words[0] == YESNO_OPTIONS[yesno_variation][0]:
                    answer = YESNO_OPTIONS[yesno_variation][1]
                    break
            #If no match to words[0] is found in YESNO_OPTIONS, answer keeps the default value None
