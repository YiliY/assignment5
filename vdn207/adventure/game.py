...
# Alternative data types for YESNO_ANSWERS
ListOfTrueCommands = ['y', 'yes']   # User command is 'yes'
ListOfFalseCommands = ['n', 'no']   # User command is 'no'
...
class Game(Data):
    ...
    def _do_command(self, words):
        if self.yesno_callback is not None:
            #answer = YESNO_ANSWERS.get(words[0], None) # Original Implementation

            
            # Using alternative data structure

            if words[0] in ListOfTrueCommands:  # If the user command is 'yes'  
                answer = True
            elif words[0] in ListOfFalseCommands:   # If the user command is 'no'
                answer = False
            else:                   # Default value of answer - None
                answer = None
