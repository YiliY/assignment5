...
YESNO_ANSWERS2 = [['y',True],['yes',True],['n',False],['no',False]]
...
class Game(Data):
    ...
    def _do_command(self, words):
        if self.yesno_callback is not None:
            ...
            #initialize answer to default value of None
            answer = None
            #for each pair in our list, check if the first element matches input
            for yesnopair in range(0,len(YESNO_ANSWERS2)): #the list YESNO_ANSWERS2 is in line 18 of this module
                if YESNO_ANSWERS2[yesnopair][0] == words[0]:
                    #if so, change answer to correct boolean value
                    answer = YESNO_ANSWERS2[yesnopair][1]
                    break
