...
class ReprTest(unittest.TestCase):
    ...
    def test_room_repr(self):
        self.assertRegexpMatches(repr(self.data.rooms[64]), '<room 64 at .*>')

    def test_object_repr(self):
        self.assertRegexpMatches(repr(self.data.objects['chest']),
                         r'<Object 55 chest/box/treasure .*>')

    def test_word_repr(self):
        self.assertRegexpMatches(repr(self.data.vocabulary['eat']), '<Word eat>')
