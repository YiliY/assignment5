...
class Game(Data):
    ...
    def _do_command(self, words):
        if self.yesno_callback is not None: 
            inputword = words[0]               
            #return the index to variable
            answerindex = next((i for i, sublist in enumerate(YESNO_ANSWERS) if inputword in sublist), -1)
            #get the coresponding value to that index in the list
            answer = YESNO_ANSWERS[answerindex][1]
            if answer is None:
