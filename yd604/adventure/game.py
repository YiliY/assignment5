...
# Modify the method to use the data type tuple
...
class Game(Data):
    ...
    def _do_command(self, words):
        if self.yesno_callback is not None:
            if ((words[0] == YESNO_ANSWERS[0][0]) or (words[0] == YESNO_ANSWERS[1][0])):
                answer = YESNO_ANSWERS[0][1]
            elif ((words[0] == YESNO_ANSWERS[2][0]) or (words[0] == YESNO_ANSWERS[3][0])):
